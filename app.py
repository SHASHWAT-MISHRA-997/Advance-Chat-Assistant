...
tabs = st.tabs([
    ...
    "📊 Text to SQL",
    "🧑‍🏫 Research Assistant",
    "🧠 Sentiment & Emotion Analysis",   # Added Brain symbol for emotion analysis
])
...
with tabs[0]:
    ...
    use_voice_response = st.checkbox("🔊 Enable Voice Responses")

    # Display chat history
    if st.session_state['history']:
        for chat in st.session_state['history']:
            st.markdown(f"**You:** {chat['user']} ({chat['timestamp']})")
            st.markdown(f"**Llama:** {chat['bot']}")

    st.markdown("---")

    user_text_input = st.text_input("Type your message here:", key="user_input_temp")

    # Voice Input for speech command
    if st.button("Speak 🎤", key="speak_button_tab_0"):
        recognized_speech = recognize_speech()
        if recognized_speech:
            st.session_state['user_input'] = recognized_speech
            st.success(f"Recognized Command: {st.session_state['user_input']}")

            # Process the voice input immediately
            with st.spinner("Thinking... 🤔"):
                bot_response = llama_chatbot(st.session_state['user_input'])
                ...
                if use_voice_response:
                    speak_text(bot_response)  # Call the speak_text function to speak the response
                    st.session_state['is_speaking'] = True  # Track speaking status

    # Check for user input from text input
    if user_text_input:
        st.session_state['user_input'] = user_text_input  # Update session state

        # Clear the temporary input from session state
        if 'user_input_temp' in st.session_state:
            del st.session_state['user_input_temp']  # Clear the temp input in session state

        # Process the text input immediately
        if st.session_state['user_input']:  # Ensure user input is not empty
            with st.spinner("Thinking... 🤔"):
                bot_response = llama_chatbot(st.session_state['user_input'])
                timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                st.session_state['history'].append({
    ...
    # Stop voice if the button is clicked and speaking is ongoing
    if st.session_state.get('is_speaking', False):
        if st.button("Stop Voice 🔇"):
            engine.stop()
            st.session_state['is_speaking'] = False  # Reset speaking status
